...
namespace L.Study.Common.Cache
{
    public abstract class ACachemgr : ICacheMgr
    {
        ...
        public abstract bool Add<T>(string key, T value, int expireByMinutes);
        ...
        public abstract bool Set<T>(string key, T value, int expireByMinutes);
        public virtual T GetOrSetThenGet<T>(string key, Func<T> getFunc) where T : class
        {
            var value = Get<T>(key);
            if (value != null)
            {
